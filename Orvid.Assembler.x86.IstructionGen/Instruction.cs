...
namespace Orvid.Assembler.x86.IstructionGen
{
	public sealed class Instruction
	{
		...
		private List<InstructionForm> FinalForms = new List<InstructionForm>(32);

		private bool TrueRegFormExists(InstructionForm parForm, int ignArgIdx, InstructionArgType regType)
		{
			bool exists = false;
			foreach (InstructionForm f in Forms.Values)
			{
				...
				{
					case 0:
						if (f.Arg2.ArgType == parForm.Arg2.ArgType &&
						    f.Arg3.ArgType == parForm.Arg3.ArgType)
						{
							if (f.Arg1.ArgType == regType)
							{
								exists = true;
								goto Return;
							}
						...
						break;
					case 1:
						if (f.Arg1.ArgType == parForm.Arg1.ArgType &&
						    f.Arg3.ArgType == parForm.Arg3.ArgType)
						{
							if (f.Arg2.ArgType == regType)
							{
								exists = true;
								goto Return;
							}
					...
					case 2:
						if (f.Arg1.ArgType == parForm.Arg1.ArgType &&
						    f.Arg2.ArgType == parForm.Arg2.ArgType)
						{
							if (f.Arg3.ArgType == regType)
							{
								exists = true;
								goto Return;
							}
		...
		public void Write(string outDir, CodeNamespace nmspc)
		{
			...
			mth.Name = "ToString";
			mth.Parameters.Add(new CodeParameterDeclarationExpression(StaticTypeReferences.x86AssemblySyntax, StaticTypeReferences.ToString_SyntaxArgName));
			mth.Attributes = MemberAttributes.Public;
			mth.Attributes |= MemberAttributes.Override;
			...
			CodeCaseStatement cs = new CodeCaseStatement(
				new CodeFieldReferenceExpression(
					StaticTypeReferences.x86AssemblySyntaxExpression,
					"NASM"
				)
			);
			if (!lonely)
