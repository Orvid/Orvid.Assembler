...
namespace Orvid.Assembler.x86.IstructionGen
{
	...
	public sealed class CustomArgOperation
	{
				...
				while (tok.Type != TokenType.RParen)
				{
					...
					switch (tok.Type)
					{
						case TokenType.Identifier:
							parms.Add(GetExpressionFromIdentifier(tok.Value, frm, argIdx, wrOp));
							break;
						case TokenType.DecimalNumber:
							int v = int.Parse(tok.Value);
							parms.Add(new CodePrimitiveExpression(v));
							break;
						case TokenType.HexadecimalNumber:
							uint val = uint.Parse(tok.Value.Substring(2), NumberStyles.AllowHexSpecifier);
							parms.Add(new CodePrimitiveExpression(val));
							break;
						default:
		...
		private CodeExpression GetExpressionFromIdentifier(string val, InstructionForm frm, int argIdx, WriteOperation wrOp)
		{
			CustomInstructionArgParameter param;
			...
			{
				return wrOp.WriteArgument.GetLoadExpression(frm);
			}
			else
			{
				...
				switch (ID)
				{
					...
					case Imm32_ID:
						prs.Add(new CodeParameterDeclarationExpression(StaticTypeReferences.UInt, arg.Name + ImmArgSuffix) 
							{
								DefaultValueExpression = (arg.DefaultValue != "" ? new CodePrimitiveExpression((int)uint.Parse(arg.DefaultValue)) : null) 
							}
						);
						return;
			...
			switch (ID)
			{
				...
				case Imm8_ID:
					prs.Add(new CodeParameterDeclarationExpression(StaticTypeReferences.Byte, arg.Name + ImmArgSuffix) 
						{
							DefaultValueExpression = (arg.DefaultValue != "" ? new CodePrimitiveExpression((int)byte.Parse(arg.DefaultValue)) : null) 
						}
					);
					return;
				case Imm16_ID:
					prs.Add(new CodeParameterDeclarationExpression(StaticTypeReferences.UShort, arg.Name + ImmArgSuffix) 
						{
							DefaultValueExpression = (arg.DefaultValue != "" ? new CodePrimitiveExpression((int)ushort.Parse(arg.DefaultValue)) : null) 
						}
					);
					return;
				case Imm32_ID:
